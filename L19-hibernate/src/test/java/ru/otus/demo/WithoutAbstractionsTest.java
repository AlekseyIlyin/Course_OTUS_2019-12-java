...
public class WithoutAbstractionsTest extends AbstractHibernateTest {
  ...
  void shouldCorrectSaveAndLoadUserWithExpectedQueriesCount(boolean userDetachedBeforeGet) {
    try (Session session = sessionFactory.openSession()) {
      ...
      assertThat(loadedUser).isNotNull().isEqualToComparingOnlyGivenFields(savedUser, "id","name","age","address");
  ...
  void shouldGenerateNewIdWhenExecuteSaveOrUpdateMethodOnSameEntity() {
    try (Session session = sessionFactory.openSession()) {
      ...
      assertThat(loadedUser).isEqualToComparingOnlyGivenFields(savedUser, "id","name","age","address");
      assertThat(loadedUser.getPhones()).containsExactlyInAnyOrderElementsOf(savedUser.getPhones());
    }
  }
  ...
  void shouldNotDetachPersistentEntityWhenRemoveWithHQLQuery() {
    ...
    try (Session session = sessionFactory.openSession()) {
      ...
      Query queryPhonesDelete = session.createQuery("delete from PhoneDataSet phone where phone.id in (?1)");
      queryPhonesDelete.setParameter(1, savedUser.getPhones().stream().map(phone -> phone.getId()).collect(Collectors.toList()));
      queryPhonesDelete.executeUpdate();
